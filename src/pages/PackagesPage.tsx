    ...
    <>
      <div className="container mx-auto mb-40">
        ...
        <section className="flex items-center">
          <BsChevronLeft size={30} onClick={sliderleft} className="hidden lg:block  opacity-20 hover:opacity-100"/>
          <div id="slider" className="mx-5 flex gap-8 w-full h-full overflow-x-scroll scroll-smooth scrollbar-hide">
            
            {allPackages.map((packageInfo, index) => (
              <PackageCardComponent key={index} packageInfo={packageInfo} />
            ))}
          </div>
          <BsChevronRight className="lg:block hidden opacity-20 hover:opacity-100" size={30} onClick={slideright}/>
        </section>
      </div>
    </>
  );
